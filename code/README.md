...
`risk/trigger/plan` 只要求非空和简短，不做枚举限制。校验器会把常见同义词归一化，例如 `harmless -> benign`、`allow -> answer`、`decline -> refuse`，最终写入数据的是归一化后的 response。
## 输出
